...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    ...
    // the first column is String, the others are Double
    val stringField = StructField(columnNames.head , StringType , nullable = false)

    val longFields = columnNames
      .tail // skip the first column which is string
      .map(column => StructField(column,DoubleType,nullable = false))

    val fields = List(stringField) ++ longFields
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val otherActivities = columnNames.partition{
      column => (
          ...
          (column.startsWith("t18") && !column.startsWith("t1801") && !column.startsWith("t1803") && !column.startsWith("t1805") )
        )
    }._1.map(x => column(x))
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = when($"telfs">0 && $"telfs"<3 ,"working").otherwise("not working").as("working")
